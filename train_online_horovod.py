...
import torch.utils.data.datapipes.iter.combinatorics as combinatorics
import torchvision
...
def train_xe(target_model, online_model, dataloader, optim, text_field, emotion_encoder=None):
    ...
    for it, (detections, captions, emotions) in enumerate(metric_logger.log_every(dataloader, _print_freq, header)):
        detections, captions = detections.cuda(), captions.cuda()
        with torch.no_grad():
                detections = image_model(detections)
...
def train_scst(target_model, online_model, dataloader, optim, cider, text_field):
    ...
    for it, (detections, captions_emotions) in enumerate(metric_logger.log_every(dataloader, _print_freq, header)):
        ...
        reward = torch.from_numpy(reward).cuda().view(detections.shape[0], beam_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feature_extractor', type=str, default=('BLIP'), choices=('CLIP', 'BLIP'))
    ...
    dataset = ArtEmis(image_field, text_field, emotion_field, args.annotation_folder, visual_encoder= 'CLIP')
    ...
    if args.use_emotion_labels:
        ...
        emotion_encoder.cuda()
    
    _logger.info("EMOTION DIMENSION IS EQUAL TO =")#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    _logger.info(emotion_dim)#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    ...
    if args.phase == 'scst':
        ...
        dataset_train = dataset_train.image_dictionary({'image': image_field, 'text': RawField()})#YOU CAN CHANGE THIS INSTEAD OF 385

    train_sampler = torch.utils.data.distributed.DistributedSampler(dataset_train, num_replicas=hvd.size(),rank=global_rank)
    scst_sampler = torch.utils.data.distributed.DistributedSampler(dict_dataset_train, num_replicas=hvd.size(),rank=global_rank)
    ...
    dataloader_scst = DataLoader(dict_dataset_train, batch_size=args.batch_size // 5, shuffle=False,
                                           num_workers=args.workers, sampler=scst_sampler, pin_memory=True)
    
    dataloader_train = DataLoader(dataset_train, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, sampler=train_sampler, pin_memory=True)
    ...
    # Create the models
    target_model = Captioner(args, text_field).cuda()
    online_model = Captioner(args, text_field).cuda()
    ...
    optim = hvd.DistributedOptimizer(optim, named_parameters=online_model.named_parameters())
    hvd.broadcast_parameters(online_model.state_dict(), root_rank=0)
    ...
    if args.resume_last or args.resume_best:
        if args.resume_last:
            fname = Path('/home/ayousefi/projects/def-kpassi/ayousefi/Epoch0_Grounded_Shuffled_Clip').joinpath(f'{args.exp_name}_last_emo_horovod.pth')
        ...
        fname = Path('/home/ayousefi/projects/def-kpassi/ayousefi/Epoch14_Grounded_Shuffled_Blip_Horovod').joinpath(f'{best_xe_model}_last_emo.pth')
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if args.rank == 0:
            ...
            if args.use_emotion_labels:
                torch.save(save_dict, f'/home/ayousefi/projects/def-kpassi/ayousefi/Epoch0_Grounded_Shuffled_Clip/{args.exp_name}_last_emo_horovod.pth')
            else:
                torch.save(save_dict, f'/home/ayousefi/projects/def-kpassi/ayousefi/Epoch0_Grounded_Shuffled_Clip/{args.exp_name}_last_emo_horovod.pth')

            if best:
                if args.use_emotion_labels:
                    copyfile(f'/home/ayousefi/projects/def-kpassi/ayousefi/Epoch0_Grounded_Shuffled_Clip/{args.exp_name}_last_emo_horovod.pth', f'/home/ayousefi/projects/def-kpassi/ayousefi/Epoch0_Grounded_Shuffled_Clip/{args.exp_name}_best_emo_horovod.pth')
                else:
                    copyfile(f'/home/ayousefi/projects/def-kpassi/ayousefi/Epoch0_Grounded_Shuffled_Clip/{args.exp_name}_last_emo_horovod.pth', f'/home/ayousefi/projects/def-kpassi/ayousefi/Epoch0_Grounded_Shuffled_Clip/{args.exp_name}_best_emo_horovod.pth')
